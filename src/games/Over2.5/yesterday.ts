...
const venasbet = async () => {
    const { data } = await axios.get(`https://venasbet.com/over-2-5-goals-prediction?dt=${d.getFullYear()}-${ d.getDate() === 1 ? d.getMonth() : d.getMonth() + 1}-${
        day
      }`);
    const html = data;
    const $ = cheerio.load(html);
...
app.get('/yesterday/games/over2.5', getGames, cache({
    cacheName: 'yesterdayover2.5',
