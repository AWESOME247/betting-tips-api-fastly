...
const meritPredict = async () => {
    const { data } = await axios.get('https://zakabet.com/double-chance-predictions/');
    const html = data;
...
const venasbet = async () => {
    const { data } = await axios.get(`https://venasbet.com/double_chance?dt=${d.getFullYear()}-${ d.getDate() === 1 ? d.getMonth() : d.getMonth() + 1}-${
        day
      }`);
    const html = data;
    const $ = cheerio.load(html);
...
app.get('/yesterday/games/double_chance', getGames, cache({
    cacheName: 'yesterdayDC',
