...
const meritPredict = async () => {
    const { data } = await axios.get('https://zakabet.com/both-team-scores-bts-gg/');
    const html = data;
...
const venasbet = async () => {
    const { data } = await axios.get(`https://venasbet.com/btts_gg?dt=${d.getFullYear()}-${ d.getDate() === 1 ? d.getMonth() : d.getMonth() + 1}-${
        day
      }`);
    const html = data;
    const $ = cheerio.load(html);
...
app.get('/yesterday/games/BTTS', getGames, cache({
